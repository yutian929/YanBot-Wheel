...
def read_csv_and_publish():
    ...
    csv_file_path = os.path.join(script_dir, 'data', 'data_imu9_1.csv')  # Build the full path
    ...
    with open(csv_file_path, 'r') as csv_file:
        ...
        for row in csv_reader:
            ...
            imu_msg.linear_acceleration.x = float(row['motionUserAccelerationX(G)']) * 9.81  # Convert G to m/s^2
            imu_msg.linear_acceleration.y = float(row['motionUserAccelerationY(G)']) * 9.81
            imu_msg.linear_acceleration.z = float(row['motionUserAccelerationZ(G)']) * 9.81
            
            # Create MagneticField message
            mag_msg = MagneticField()
            mag_msg.header = header  # Same timestamp and frame as IMU
            mag_msg.magnetic_field.x = float(row['motionMagneticFieldX(µT)']) * 1e-6  # Convert µT to Tesla
